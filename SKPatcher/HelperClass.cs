...
namespace SKPatcher
{
	public class HelperClass
	{
		...
		public static GenericFuncContainer<T,bool> MemberCustomAttributeComparer<T>(params string[] attributeClasses) where T : IMemberDefinition
		{
			return new GenericFuncContainer<T,bool>(member => {
				Dictionary<string,bool> attributeChecklist = new Dictionary<string, bool>();
				foreach (string attributeClass in attributeClasses)
					attributeChecklist.Add(attributeClass, false);
				foreach (CustomAttribute curAttr in member.CustomAttributes)
				{
					string curAttributeClass = curAttr.AttributeType.FullName;
					if (attributeChecklist.ContainsKey(curAttributeClass))
						attributeChecklist[curAttributeClass] = true;
				}
				return !(attributeChecklist.ContainsValue(false));
			});
		}
