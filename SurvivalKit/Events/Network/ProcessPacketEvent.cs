...
namespace SurvivalKit.Events.Network
{
	...
	/// </summary>
	public class ProcessPacketEvent : Event, ICancellable
	{
		...
		public ProcessPacketEvent(Object[] args)
		{
			...
			if (miPkType == null || !typeof(PackageType).IsAssignableFrom(miPkType.ReturnType)) {
				UnityEngine.Debug.LogError("A packet of type '" + this.packet.GetType().FullName + "' hasn't got a valid GetPkType() function!");
			}
			...
			foreach (Event curEvent in subevents) {
